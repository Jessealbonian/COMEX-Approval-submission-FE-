...
type DocRow = {
  ...
  submittedOn: Date;
  coordChecked: string;
  coordStatus: 'Pending' | 'Checked' | 'For Revision';
  masterChecked: string;
  masterStatus: 'Pending' | 'Checked' | 'For Revision';
  principalChecked: string;
  principalStatus: 'Pending' | 'Checked' | 'For Revision';
  revisions: string;
};
...
@Component({
  ...
  styleUrl: './documenttable.component.css'
})
export class DocumenttableComponent {
  ...
  openDocument(row: DocRow): void {
    // Front-end only: route to the viewer.
    // If you later add backend, you can pass an ID here.
    void this.router.navigateByUrl('/document');
  }
}
